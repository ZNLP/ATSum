...
Batch = namedtuple("Batch", ['src', 'tgt', 'batch_size', 'probs', 'idxes'])
    ...
    def __iter__(self):
        while True:
            ...
            for minibatch in self.batches:
                src, probs, idxes = self.fields["src"].process([x.src for x in minibatch], self.device, 'src')
                tgt = self.fields["tgt"].process([x.tgt for x in minibatch], self.device, 'tgt')
                yield Batch(src=src, tgt=tgt, batch_size=len(minibatch), probs=probs, idxes=idxes)
            if not self.train:
                break
