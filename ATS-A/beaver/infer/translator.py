...
def beam_search(opt, model, src, probs, idxes, fields):
    ...
    previous = None
    trans_probs = transattn(src_out, idxes)

    ######
    # trans_probs = trans_probs * probs
    ...
    for i in range(opt.max_length):
        ...
        out, previous, p_trans, weights = decoder(current_token, src_out, src_pad, tgt_pad, previous, i)
        previous_score = torch.stack([b.scores for b in beams]).unsqueeze(-1)
        out = generator(out)
        bsize, _, tgtvocab = out.size()
        _, srclen, _ = src_out.size()
        ...
        out = torch.log(p_trans * trans_scores + (1 - p_trans) * out)
