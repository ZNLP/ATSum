...
def translate(dataset, fields, model):
    ...
    for batch in dataset:
        if opt.tf:
            ...
            predictions = beam_search(opt, model, batch.src, batch.probs, batch.idxes, fields)
...
def main():
    ...
    dataset = build_dataset(opt, [opt.input, opt.truth or opt.input], opt.vocab, device, prob_and_idx, train=False)
